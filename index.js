...
client.on("messageCreate", async (message) => {
  ...
  if (comando === "!ticket") {
    ...
    const embed = new EmbedBuilder()
      .setColor("#b20710")
      .setAuthor({
        name: `${message.guild.name} • Atendimento`,
        iconURL: message.guild.iconURL({ dynamic: true })
      })
      .setTitle("🎫 Central de Atendimento")
      .setDescription(
        `> Olá! Seja bem-vindo(a) ao suporte oficial.\n\n` +
        `Clique no botão abaixo para abrir um ticket privado com nossa equipe de atendimento.`
      )
  ...
  if (interaction.customId === "abrir_ticket") {
    ...
    try {
      ...
      const embedTicket = new EmbedBuilder()
        ...
        .setDescription(
          `Olá ${interaction.user}, seja bem-vindo(a) ao atendimento.\n\n` +
          `Explique sua dúvida ou problema com o máximo de detalhes possível.\n` +
          `Nossa equipe irá te responder em breve.`
        )
        .setFooter({
          text: "Para encerrar o atendimento, clique em Fechar Ticket."
        })
        .setTimestamp();

      await canal.send({
        content: config.cargoTicket ? `<@&${config.cargoTicket}>` : "",
        embeds: [embedTicket],
